...
def generate_mesh_dependency_html_with_search(dependencies, reverse_dependencies):
    ...
    for source_page, linked_pages in dependencies.items():
        ...
        for target_page in linked_pages:
            ...
            html += f'{{from: {source_id}, to: {target_id}, arrows: "to", color: {{color: "#cccccc"}}}},\n'  # Uniform color for links
    ...
    // Add click event to nodes to highlight immediate neighbors
    network.on("click", function (params) {
        if (params.nodes.length > 0) {
            ...
            // Get the immediate neighbors (connected nodes)
            var connectedNodeIds = [];
            edges.forEach(function(edge) {
                if (edge.from === clickedNodeId) {
                    connectedNodeIds.push(edge.to);  // Node connected to the clicked node
                } else if (edge.to === clickedNodeId) {
                    connectedNodeIds.push(edge.from);  // Node connected from the clicked node
                }
            });

            // Include immediate neighbors in the nodes to remain highlighted
            var highlightNodeIds = [clickedNodeId].concat(connectedNodeIds);

            // Update nodes and edges to dim the ones not in the result or immediate neighbors
            nodes.update(nodes.get().map(function(node) {
                if (highlightNodeIds.includes(node.id)) {
                    return {
                        id: node.id,
                        color: {
                            background: '#97C2FC',  // Regular color
                            border: '#2B7CE9',      // Regular border color
                            opacity: 1              // Full opacity
                        },
                        font: { color: '#000000' }   // Regular font color
                    };
                } else {
                    return {
                        id: node.id,
                        color: {
                            background: '#cccccc',  // Faded background color
                            border: '#cccccc',      // Faded border color
                            opacity: 0.1            // Reduced opacity
                        },
                        font: { color: '#cccccc' }   // Faded font color
                    };
                }
            }));

            edges.update(edges.get().map(function(edge) {
                if (highlightNodeIds.includes(edge.from) || highlightNodeIds.includes(edge.to)) {
                    return {
                        id: edge.id,
                        color: { color: '#6495ED', opacity: 1 } // Full opacity and regular color
                    };
                } else {
                    return {
                        id: edge.id,
                        color: { color: '#cccccc', opacity: 0.1 } // Faded color and reduced opacity
                    };
                }
            }));
        } else {
            // If no nodes are selected or click is cleared, reset everything
            network.unselectAll();
            nodes.update(nodes.get().map(function(node) {
                return {
                    id: node.id,
                    color: {
                        background: '#97C2FC',  // Regular background color
                        border: '#2B7CE9',      // Regular border color
                        opacity: 1              // Full opacity
                    },
                    font: { color: '#000000' }   // Regular font color
            ...
            edges.update(edges.get().map(function(edge) {
                return {
                    id: edge.id,
                    color: { color: '#6495ED', opacity: 1 } // Regular color and opacity
                };
            }));
        }
    });
...
function searchNode() {
    ...
    // Get all nodes that match the search input
    ...
    // If there are matching nodes, highlight them and blur the rest
    if (foundNodes.length > 0) {
        ...
        edges.forEach(function(edge) {
            if (nodeIds.includes(edge.from)) {
                connectedNodeIds.push(edge.to);  // Node connected to the found node
            } else if (nodeIds.includes(edge.to)) {
                connectedNodeIds.push(edge.from);  // Node connected from the found node
            }
        });

        // Include immediate neighbors in the nodes to remain highlighted
        var highlightNodeIds = nodeIds.concat(connectedNodeIds);

        // Update nodes and edges to dim the ones not in the result or immediate neighbors
        nodes.update(nodes.get().map(function(node) {
            if (highlightNodeIds.includes(node.id)) {
                // Highlight matching nodes and their immediate neighbors (normal state)
                return {
                    id: node.id,
                    color: {
                        background: '#97C2FC',  // Regular color
                ...
                // Dim non-matching nodes (faded out state)
                return {
                    id: node.id,
                    color: {
                        background: '#cccccc',  // Faded background color
        ...
        edges.update(edges.get().map(function(edge) {
            if (highlightNodeIds.includes(edge.from) || highlightNodeIds.includes(edge.to)) {
                // Keep edges connected to the found nodes or their immediate neighbors (normal state)
                return {
                    id: edge.id,
                    color: { color: '#6495ED', opacity: 1 } // Full opacity and regular color
                };
            } else {
                // Dim edges not connected to the found nodes or immediate neighbors (faded out state)
                return {
                    id: edge.id,
                    color: { color: '#cccccc', opacity: 0.1 } // Faded color and reduced opacity
                };
        ...
        // If no nodes are found or search is cleared, reset everything
        network.unselectAll();

        // Restore nodes and edges to full opacity and original color
        nodes.update(nodes.get().map(function(node) {
            return {
                id: node.id,
                color: {
